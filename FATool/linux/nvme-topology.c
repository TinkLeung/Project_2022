...
int get_nsid(int fd)
{
	...
	if (nsid <= 0) {
        fprintf(stderr,	"%s: failed to return namespace id\n",devicename);
	}
...
int scan_namespace(struct nvme_namespace *n)
{
	int ret, fd;
	char *path;
...
static int legacy_list(struct nvme_topology *t)
{
	...
	for (i = 0; i < t->nr_subsystems; i++) {
        ...
        //ret = asprintf(&path, "%s%s", dev, c->name); compile error
		if (ret < 0)
			continue;
		ret = 0;
					 ...
					 __u32 nsid)
{
	...
	for (i = 0; i < n; i++) {
        snprintf(path, sizeof(path), "%s%s", subsys_dir,subsys[i]->d_name);
