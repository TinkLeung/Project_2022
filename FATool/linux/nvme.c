...
static int discover_cmd(int argc, char **argv, struct command *command, struct plugin *plugin)
{
    ...
    return 0; //return discover(desc, argc, argv, false); compile error
}
...
static int connect_all_cmd(int argc, char **argv, struct command *command, struct plugin *plugin)
{
    ...
    return 0; //return discover(desc, argc, argv, true); compile error
}
...
static int disconnect_cmd(int argc, char **argv, struct command *command, struct plugin *plugin)
{
    ...
    return 0; //return disconnect(desc, argc, argv); compile error
}
...
static int disconnect_all_cmd(int argc, char **argv, struct command *command, struct plugin *plugin)
{
    ...
    return 0; //return disconnect_all(desc, argc, argv);
}
...
int fw_download_ui(char* nvmedev, char* FileName, __u32 transize)
{
    ...
    char *device;
    asprintf(&device, "%s%s", dev, nvmedev);
    fd = open_dev(device);
    if (fd < 0) {
            err = fd;
            goto ret;
...
int fw_commit_ui(char* nvmedev, int slotID, unsigned long selectAction)
{
    ...
    cfg.slot = (unsigned char)slotID;
    cfg.action = (unsigned char)selectAction;
    char *device;
    asprintf(&device, "%s%s", dev, nvmedev);
    fd = open_dev(device);
    if (fd < 0) {
