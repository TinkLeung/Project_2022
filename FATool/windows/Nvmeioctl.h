...
typedef struct {
    ...
    UCHAR   NWPC;                // byte 531.   M - Namespace Write Protection Capabilities (NWPC)

    USHORT  ACWU;               // byte 532:533  O - Atomic Compare & Write Unit (ACWU)

    UCHAR   Reserved2[2];       // byte 534:535.
    ...
    UCHAR   Reserved3[160];     // byte 544:703.

    //
    // byte 704 : 2047, I/O Command Set Attributes
    //
    UCHAR   Reserved4[64];      // byte 704:767.
    UCHAR   SUBNQN[256];        // byte 768:1023. O - NVM Subsystem NVMe Qualified Name (SUBNQN)
    UCHAR   Reserved5[1024];    // byte 1024:2047.
...
typedef struct {
   ...
   union {
    ...
    UCHAR           DLFEAT;             // byte 33 O - Deallocate Logical Block Features (DLFEAT)
    ...
    USHORT          NOIOB;              // byte 46:47 O - Namespace Optimal I/O Boundary (NOIOB)

    UCHAR           NVMCAP[16];         // byte 48:63 O - NVM Capacity (NVMCAP)

    USHORT          NPWG;               // byte 64:65 O - Namespace Preferred Write Granularity (NPWG)
    USHORT          NPWA;               // byte 64:67 O - Namespace Preferred Write Alignment (NPWA)
    ...
    UCHAR           Reserved0[18];      // byte 74:91

    ULONG           ANAGRPID;           // byte 92:95 O - ANA Group Identifier
    UCHAR           Reserved1[3];       // byte 96:98
    UCHAR           NSATTR;             // byte 99 O - Namespace Attributes
    USHORT          NVMSETID;           // byte 100:101 O - Namespace Set Identifier
    USHORT          ENDGID;             // byte 102:103 O - Endurance Group Identifier
